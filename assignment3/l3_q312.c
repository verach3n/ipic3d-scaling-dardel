...
int main(int argc, char *argv[]){
    ...
    MPI_Irecv(&f[0], 1, MPI_DOUBLE, (rank-1+size)%size, 0, MPI_COMM_WORLD, &requests[0]);
    MPI_Irecv(&f[nxn_loc-1], 1, MPI_DOUBLE, (rank+1+size)%size, 0, MPI_COMM_WORLD, &requests[1]);
    MPI_Isend(&f[nxn_loc-3], 1, MPI_DOUBLE, (rank+1+size)%size, 0, MPI_COMM_WORLD, &requests[2]);
    MPI_Isend(&f[1], 1, MPI_DOUBLE, (rank-1+size)%size, 0, MPI_COMM_WORLD, &requests[3]);
